...
import { listUsers, listWorkspaces } from "@/db/foundation";

export default async function AdminPage() {
  const currentUser = await requireInternalAdmin();
  const [users, workspaces] = await Promise.all([listUsers(), listWorkspaces()]);
  ...
  return (
    <div className="page-shell space-y-8 pt-0">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatusCard
          label="Storage"
          value={env.hasNeon ? "Neon database" : "Local preview store"}
        />
        <StatusCard
          label="Billing"
                ...
                workspaces.map((workspace) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        <p className="font-semibold">{owner?.fullName ?? "Unknown owner"}</p>
                        <p className="text-muted">{owner?.email ?? workspace.ownerUserId}</p>
                      </td>
